<template>
  <transition name="panierr">
    <div
      v-if="show"
      class="w-full fixed top-4 z-[51] lg:left-80 md:left-36 max-w-2xl rounded-xl h-[28rem] bg-white p-6 md:p-12 mt-16"
    >
      ...
      <div class="space-y-2 h-[20rem] flex flex-col w-full">
        ...
        <div class="flex justify-end">
          <button
            @click="PayerCommande(), closeModal()"
            class="inline-flex px-4 mt-8 py-2 text-white items-center justify-center shrink-0 font-bold leading-none rounded outline-none transition duration-300 ease-in-out focus:outline-none focus:shadow focus:ring-1 focus:ring-accent-700 bg-vert text-light border border-transparent hover:bg-accent-hover"
          >
            Payer
          </button>
        </div>
