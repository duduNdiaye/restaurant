            ...
            >
              <div class="flex">
                <span class="text-lg lg:text-xl font-semibold text-heading md:text-base">
                  {{ user.name }}</span
                >
                <!-- <div class="flex ml-auto shrink-0 items-center text-sm text-white">
                  <div class="inline-flex" >
                    <span

                      @mouseover="Noter(i = 1),temoin=user.id,temoin1 = user.id,temoin2=null"
                      @mouseleave="temoin1=temoin2,temoin=temoin2"
                      @click="Note(i1), temoin2 = user.id"
                      :class="[i1 &lt;= nombre && temoin == user.id  ?  'bg-vert':'bg-white']"
                       class="mr-1  rounded-full w-4 h-4 border-2 border-vert"
                    ></span>
                    <span
                      @mouseover="Noter(i = 2),temoin=user.id,temoin22 = user.id,temoin2=null"
                       @mouseleave="temoin22=temoin2,temoin=temoin2"
                      @click="Note(i2), temoin2 = user.id"
                      :class="[i2 &lt;= nombre &&  temoin == user.id  ?  'bg-vert':'bg-white']"
                      class="mr-1  rounded-full w-4 h-4 border-2 border-vert"
                    ></span>
                    <span
                      @mouseover="Noter(i = 3),temoin=user.id,temoin3 = user.id,temoin2=null"
                         @mouseleave="temoin3=temoin2,temoin=temoin2"
                      @click="Note(i3), temoin2 = user.id"
                      :class="[i3 &lt;= nombre && temoin == user.id  ?  'bg-vert':'bg-white']"
                      class="mr-1  rounded-full w-4 h-4 border-2 border-vert"
                    ></span>
                    <span
                      @mouseover="Noter(i = 4),temoin=user.id,temoin4 = user.id,temoin2=null"
                      @mouseleave="temoin4=temoin2,temoin=temoin2"
                      @click="Note(i4), temoin2 = user.id"
                      :class="[i4 &lt;= nombre && temoin == user.id  ?  'bg-vert':'bg-white']"
                      class="mr-1  rounded-full w-4 h-4 border-2 border-vert"
                    ></span>
                    <span
                      @mouseover="Noter(i = 5),temoin=user.id,temoin5 = user.id,temoin2=null"
                      @mouseleave="temoin5=temoin2,temoin=temoin2"
                      @click="Note(i5), temoin2 = user.id"
                      :class="[i5 &lt;= nombre && temoin == user.id ? 'bg-vert':'bg-white']"
                      class="mr-1  rounded-full w-4 h-4 border-2 border-vert"
                    ></span>
                  </div>
                </div> -->
                <div class="flex space-x-1 bg-vert rounded-full px-2 ml-auto mt-2">
                    ...
                    <span class="text-black font-bold text-sm flex items-center justify-center">3.4</span>
                </div>
              </div>
              <div class="flex ">
